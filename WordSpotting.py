...
def pyramid_scan(img_path, output_path, spotter, num_layers = 1, min_patch_resolution = 384, max_patch_resolution = 2048, spotting_batch_size = 8, save_visualization=False, save_results=True):
    image = Image.open(img_path)
    image_preprocessor = ImagePreprocessor(image, overlapping_tolerance=0.3, num_layers=num_layers, min_patch_resolution=min_patch_resolution, max_patch_resolution=max_patch_resolution)
    ...
    for i in range(image_preprocessor.num_layers):
        ...
        results = spotter.inference_batch(batch_size=spotting_batch_size, rotations = [0])
